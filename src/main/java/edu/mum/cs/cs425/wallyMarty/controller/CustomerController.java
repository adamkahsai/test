...
public class CustomerController {
    ...
    @GetMapping(value = {"/", "/hawkeyeSupermart"})
    ...
    public ModelAndView displayListOfCustomers(ModelAndView model) {
    ...
    public String displayListOfPrimeCustomers(Model model) {

        List<Customer> customers = customertService.getListCustomer().stream()
                .filter(c -> Period.between(c.getDateOfBirth(), LocalDate.now()).getYears() >= 40)
